  ...
  <div class="qy-hp-mainAR">
    ...
    <div class="qy-leaguerx-rightPlate">
      <div class="qy-leaguer-rplateCont">
        ...
        <form id="frmOptions" action="/index.php/activity/my_launch/delete_activity/" method="post">
        
        {foreach from=$list item=v key=k}
          <li>
            <input rel="actopt" class="qy-mpactive-secbox" name="actopt[]" type="checkbox" value="{$v.aid}" />
            {if $v.audit=='2'}<div class="qy-mpactive-chkstate">审核中</div>{/if}
            {if $v.audit=='0'}<div class="qy-mpactive-chkstate statepermit">通过</div>{/if}
            {if $v.audit=='1'}<div class="qy-mpactive-chkstate statewarm">警告</div>{/if}
            <div class="qy-mactive-detail">
              ...
              <p class="mactive-edition">发布日期：{$v['dateline']|date_format:"%Y/%m/%d"} <span class="mactive-oper" style="display:none"><a href="/index.php/activity/index/edit_activity/{$v.aid}" target="_blank">编辑</a>&nbsp;&nbsp; </span></p>
            </div>
          </li>
        {/foreach}       
