      ...
      <div class="qy-leaguer-rplateCont">
       <div class="qy-mpcommonTag-headertit"> <b class="qy-mpcommonTag-caption">交友动态</b>
          <ul class="qy-mpcommonTag-switchTags">
            <li class="cursection"><a href="/index.php/friend/index">我的好友</a></li>
            <li><a href="/index.php/friend/index/attention">我的关注</a></li>
            <li><a href="#">我的奇遇</a></li>
          </ul>
        </div>
        ...
        {foreach from=$friend_list item=v key=k}
          <li>
            <input class="chkbox" name="friendId[]" type="checkbox" value="{$v['fusuerid']}" />
            <input class="chkbox" name="keyId" type="hidden" value="0" />
            <a href="/index.php/member_info/index/uid/{$v['fusuerid']}" target="_blank" ><img src="{$v.avatar_img}" class="avatar"></a>
            <div class="favoritedetail">
              <p class="favoritename"><a href="/index.php/member_info/index/uid/{$v['fusuerid']}" target="_blank">{$v['fusername']}</a></p>
              <p class="favoritinfo"> {if $v.gender=='0'}男{else}女{/if} {$v.age}岁 {$v.city['city']} {$v.work}</p>
              <p class="favoritdesc">{$v.cut_description}</p>
              <p class="favoriteoper"><a href="/index.php/member/my_email/send_email/{$v['fusuerid']}" target="_blank" class="btn-sendto">写信给Ta</a></p>
            </div>
          </li>
        {/foreach}  
       	</form>
